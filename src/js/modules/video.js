	...
	function resizeVideoBg() {
		$('.js-video').each(function () {
			if ($(this).attr('data-video') !== undefined) {
				...
				let video = $(
					'<video autoplay="true" loop="true" preload="auto" muted="muted" playsinline><source src="' +
				...
				if (prp > prp_v) {
					...
					video.css(
						'left',
						($(this).outerWidth() - $('.js-video video').width()) / 2
					)
					video.css('top', 0)
				} else {
					...
					video.css(
						'top',
						($(this).outerHeight() - $('.js-video video').height()) / 2
					)
					video.css('left', 0)
				}
				$(this).prepend(video)
